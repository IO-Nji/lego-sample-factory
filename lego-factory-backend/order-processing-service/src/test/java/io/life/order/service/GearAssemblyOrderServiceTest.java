...
/**
 * Unit tests for GearAssemblyOrderService
 * 
 * Tests WS-4 (Gear Assembly) workstation order lifecycle:
...
@DisplayName("GearAssemblyOrderService Tests")
class GearAssemblyOrderServiceTest {
    ...
    @InjectMocks
    private GearAssemblyOrderService gearAssemblyOrderService;

    private GearAssemblyOrder testOrder;

    @BeforeEach
    void setUp() {
        testOrder = GearAssemblyOrder.builder()
                .id(1L)
                .orderNumber("GAO-001")
                .assemblyControlOrderId(200L)
                .workstationId(4L)
                .requiredPartIds("[20, 21, 22]")
                .requiredPartDetails("{\"parts\": [{\"id\": 20, \"name\": \"Gear\", \"qty\": 5}]}")
                .outputModuleId(50L)
                .outputModuleName("Gear Module Assembly")
                .quantity(25)
                .status("PENDING")
                .priority("NORMAL")
                .build();
    ...
    class GetOrdersForWorkstation {
        ...
        @Test
        @DisplayName("Should return orders for workstation 4")
        void shouldReturnOrdersForWorkstation() {
            GearAssemblyOrder order2 = GearAssemblyOrder.builder()
                    .id(2L)
                    .orderNumber("GAO-002")
                    .workstationId(4L)
                    .status("IN_PROGRESS")
            ...
            when(gearAssemblyOrderRepository.findByWorkstationId(4L))
                    .thenReturn(Arrays.asList(testOrder, order2));

            List<GearAssemblyOrder> result = gearAssemblyOrderService.getOrdersForWorkstation(4L);

            assertThat(result).hasSize(2);
            assertThat(result).extracting(GearAssemblyOrder::getOrderNumber)
                    .containsExactly("GAO-001", "GAO-002");
            verify(gearAssemblyOrderRepository).findByWorkstationId(4L);
        }
    ...
    class GetOrdersByControlOrder {
        ...
        void shouldReturnOrdersForControlOrder() {
            when(gearAssemblyOrderRepository.findByAssemblyControlOrderId(200L))
                    .thenReturn(List.of(testOrder));

            List<GearAssemblyOrder> result = gearAssemblyOrderService.getOrdersByControlOrder(200L);

            assertThat(result).hasSize(1);
            assertThat(result.get(0).getAssemblyControlOrderId()).isEqualTo(200L);
            verify(gearAssemblyOrderRepository).findByAssemblyControlOrderId(200L);
        }
    }
    ...
    class GetOrderById {
        ...
        void shouldReturnOrderWhenFound() {
            when(gearAssemblyOrderRepository.findById(1L))
                    .thenReturn(Optional.of(testOrder));

            GearAssemblyOrder result = gearAssemblyOrderService.getOrderById(1L);

            assertThat(result).isNotNull();
            assertThat(result.getOrderNumber()).isEqualTo("GAO-001");
        }

        @Test
        @DisplayName("Should throw exception when order not found")
        void shouldThrowExceptionWhenNotFound() {
            when(gearAssemblyOrderRepository.findById(999L))
                    .thenReturn(Optional.empty());

            assertThatThrownBy(() -> gearAssemblyOrderService.getOrderById(999L))
                    .isInstanceOf(RuntimeException.class)
                    .hasMessageContaining("Gear assembly order not found: 999");
        }
    }
    ...
    class StartOrder {
        ...
        void shouldStartPendingOrder() {
            ...
            when(gearAssemblyOrderRepository.findById(1L))
                    .thenReturn(Optional.of(testOrder));
            when(gearAssemblyOrderRepository.save(any(GearAssemblyOrder.class)))
                    .thenAnswer(inv -> inv.getArgument(0));

            GearAssemblyOrder result = gearAssemblyOrderService.startOrder(1L);
        ...
        void shouldThrowExceptionWhenStartingCompletedOrder() {
            ...
            assertThatThrownBy(() -> gearAssemblyOrderService.startOrder(1L))
                    .isInstanceOf(IllegalStateException.class)
                    .hasMessageContaining("Can only start PENDING or WAITING_FOR_PARTS orders");
        }

        @Test
        @DisplayName("Should generate correct SimAL task ID for WS-4")
        void shouldGenerateCorrectSimalTaskId() {
            ...
            gearAssemblyOrderService.startOrder(1L);

            String expectedTaskId = SimalClient.generateTaskId(4L, "GAO-001");
    ...
    class CompleteOrder {
        ...
        void shouldCompleteOrderAndCreditInventory() {
            ...
            GearAssemblyOrder result = gearAssemblyOrderService.completeOrder(1L);

            assertThat(result.getStatus()).isEqualTo("COMPLETED");
            assertThat(result.getActualFinishTime()).isNotNull();

            // Verify inventory credit - WS-4 produces MODULES (not parts)
            verify(inventoryClient).creditModulesToModulesSupermarket(
                    eq(50L),  // outputModuleId
                    eq(25),   // quantity
                    eq("GAO-001")  // orderNumber
            );
        }
        ...
        void shouldUpdateSimalOnCompletion() {
            ...
            gearAssemblyOrderService.completeOrder(1L);
        ...
        void shouldPropagateStatusToParent() {
            ...
            verify(orchestrationService).notifyWorkstationOrderComplete(
                    eq(OrderOrchestrationService.WorkstationOrderType.GEAR_ASSEMBLY),
        ...
        void shouldThrowExceptionWhenCompletingNonInProgressOrder() {
            ...
            assertThatThrownBy(() -> gearAssemblyOrderService.completeOrder(1L))
                    .isInstanceOf(IllegalStateException.class)
    ...
    class HaltOrder {
        ...
        void shouldHaltInProgressOrder() {
            ...
            GearAssemblyOrder result = gearAssemblyOrderService.haltOrder(1L);
        ...
        void shouldThrowExceptionWhenHaltingNonInProgressOrder() {
            ...
            assertThatThrownBy(() -> gearAssemblyOrderService.haltOrder(1L))
                    .isInstanceOf(IllegalStateException.class)
    ...
    class ResumeOrder {
        ...
        void shouldResumeHaltedOrder() {
            ...
            GearAssemblyOrder result = gearAssemblyOrderService.resumeOrder(1L);
        ...
        void shouldThrowExceptionWhenResumingNonHaltedOrder() {
            ...
            assertThatThrownBy(() -> gearAssemblyOrderService.resumeOrder(1L))
                    .isInstanceOf(IllegalStateException.class)
    ...
    class MarkWaitingForParts {
        ...
        void shouldMarkOrderAsWaitingForParts() {
            ...
            GearAssemblyOrder result = gearAssemblyOrderService.markWaitingForParts(1L, 500L);
        ...
        void shouldThrowExceptionWhenOrderNotFound() {
            ...
            assertThatThrownBy(() -> gearAssemblyOrderService.markWaitingForParts(999L, 500L))
                    .isInstanceOf(RuntimeException.class)
                    .hasMessageContaining("Gear assembly order not found");
        }
    }
    ...
    class ErrorHandling {
        ...
        void shouldHandleRepositoryException() {
            ...
            assertThatThrownBy(() -> gearAssemblyOrderService.getOrderById(1L))
                    .isInstanceOf(RuntimeException.class)
