...
if [ "$WO_TRIGGER" == "PRODUCTION_REQUIRED" ]; then
    ...
    PRODUCTION_STATUS=$(echo $PROD_LIST | jq -r 'if type == "array" then (.[0] | .status) else .status end')
    
    if [ "$PRODUCTION_ORDER_ID" != "null" ] && [ -n "$PRODUCTION_ORDER_ID" ]; then
        print_result 0 "Existing production order found ($PRODUCTION_ORDER_NUMBER, Status: $PRODUCTION_STATUS)"
    else
...
# ==========================================
# TEST 3a: CONFIRM & SUBMIT TO SIMAL
# ==========================================

if [ "$PRODUCTION_ORDER_ID" != "null" ] && [ -n "$PRODUCTION_ORDER_ID" ]; then
    print_test "3a" "Production Order Confirmation & SimAL Scheduling"
    
    # Login as Production Planning
    print_step "Login as Production Planning"
    LOGIN_PP=$(curl -s -X POST "$BASE_URL/auth/login" \
        -H "Content-Type: application/json" \
        -d '{"username":"production_planning","password":"password"}')
    
    TOKEN_PP=$(echo $LOGIN_PP | jq -r '.token')
    
    if [ "$TOKEN_PP" != "null" ] && [ "$TOKEN_PP" != "" ]; then
        print_result 0 "Login as Production Planning"
    else
        print_result 1 "Production Planning login failed"
        echo "Response: $LOGIN_PP"
    fi
    
    # Confirm production order if needed
    if [ "$PRODUCTION_STATUS" == "CREATED" ]; then
        print_step "Confirm production order (CREATED → CONFIRMED)"
        PROD_CONFIRM=$(curl -s -X PUT "$BASE_URL/production-planning/$PRODUCTION_ORDER_ID/confirm" \
            -H "Authorization: Bearer $TOKEN_PP" \
            -H "Content-Type: application/json")
        
        PRODUCTION_STATUS=$(echo $PROD_CONFIRM | jq -r '.status')
        
        if [ "$PRODUCTION_STATUS" == "CONFIRMED" ]; then
            print_result 0 "Production order confirmed (Status: CONFIRMED)"
        else
            print_result 1 "Production order confirmation failed (Got: $PRODUCTION_STATUS)"
            echo "Response: $PROD_CONFIRM"
        fi
    else
        print_info "Production order already in status: $PRODUCTION_STATUS"
    fi
    
    # Submit to SimAL for scheduling
    if [ "$PRODUCTION_STATUS" == "CONFIRMED" ] || [ "$PRODUCTION_STATUS" == "CREATED" ]; then
        print_step "Submit to SimAL for scheduling (triggers module workstation assignment)"
        SIMAL_SUBMIT=$(curl -s -X POST "$BASE_URL/production-planning/$PRODUCTION_ORDER_ID/submit-to-simal" \
            -H "Authorization: Bearer $TOKEN_PP" \
            -H "Content-Type: application/json")
        
        SIMAL_SCHEDULE_ID=$(echo $SIMAL_SUBMIT | jq -r '.simalScheduleId')
        PRODUCTION_STATUS=$(echo $SIMAL_SUBMIT | jq -r '.status')
        ESTIMATED_DURATION=$(echo $SIMAL_SUBMIT | jq -r '.estimatedDuration')
        
        if [ "$SIMAL_SCHEDULE_ID" != "null" ] && [ "$SIMAL_SCHEDULE_ID" != "" ]; then
            print_result 0 "Submitted to SimAL (Schedule: $SIMAL_SCHEDULE_ID, Status: $PRODUCTION_STATUS, Duration: ${ESTIMATED_DURATION}min)"
            print_info "SimAL is now creating tasks based on module productionWorkstationId"
        else
            print_result 1 "SimAL submission failed"
            echo "Response: $SIMAL_SUBMIT"
        fi
        
        # Wait a moment for SimAL to process
        sleep 2
        
        # Get scheduled tasks from SimAL
        if [ "$SIMAL_SCHEDULE_ID" != "null" ] && [ "$SIMAL_SCHEDULE_ID" != "" ]; then
            print_step "Fetch scheduled tasks from SimAL"
            SIMAL_TASKS=$(curl -s -X GET "$BASE_URL/production-planning/scheduled-tasks/$SIMAL_SCHEDULE_ID" \
                -H "Authorization: Bearer $TOKEN_PP")
            
            TASK_COUNT=$(echo $SIMAL_TASKS | jq 'length')
            
            if [ "$TASK_COUNT" -gt 0 ]; then
                print_result 0 "SimAL generated $TASK_COUNT tasks"
                print_info "Task breakdown:"
                echo "$SIMAL_TASKS" | jq -r '.[] | "  - \(.taskName) at \(.workstationId) (\(.duration)min)"'
            else
                print_result 1 "No tasks generated by SimAL"
                echo "Response: $SIMAL_TASKS"
            fi
        fi
        
        # Dispatch the schedule (creates control orders)
        if [ "$SIMAL_SCHEDULE_ID" != "null" ] && [ "$SIMAL_SCHEDULE_ID" != "" ]; then
            print_step "Dispatch schedule to control stations"
            DISPATCH=$(curl -s -X POST "$BASE_URL/simal/scheduled-orders/$SIMAL_SCHEDULE_ID/dispatch" \
                -H "Authorization: Bearer $TOKEN_PP" \
                -H "Content-Type: application/json")
            
            DISPATCH_STATUS=$(echo $DISPATCH | jq -r '.status // "unknown"')
            
            if [ "$DISPATCH_STATUS" == "DISPATCHED" ] || [ "$DISPATCH_STATUS" == "IN_PROGRESS" ]; then
                print_result 0 "Schedule dispatched to control stations (Status: $DISPATCH_STATUS)"
                print_info "Control orders should now be created for both Production Control and Assembly Control"
            else
                print_result 1 "Schedule dispatch failed (Status: $DISPATCH_STATUS)"
                echo "Response: $DISPATCH"
            fi
            
            # Wait for control orders to be created
            sleep 2
        fi
    else
        print_info "Production order status is $PRODUCTION_STATUS - skipping SimAL submission"
    fi
fi
...
if [ "$PRODUCTION_ORDER_ID" != "null" ] && [ -n "$PRODUCTION_ORDER_ID" ]; then
    ...
    print_step "Check for Assembly Control orders (created by SimAL dispatch)"
    ACO_LIST=$(curl -s -X GET "$BASE_URL/assembly-control-orders" \
        -H "Authorization: Bearer $TOKEN_AC")
    
    ACO_COUNT=$(echo $ACO_LIST | jq 'if type == "array" then length else 0 end')
    ACO_ID=$(echo $ACO_LIST | jq -r 'if type == "array" then (last | .id) else .id end')
    
    if [ "$ACO_COUNT" -gt 0 ] && [ "$ACO_ID" != "null" ] && [ -n "$ACO_ID" ]; then
        print_result 0 "Assembly Control Orders found: $ACO_COUNT orders"
        
        ACO_STATUS=$(echo $ACO_LIST | jq -r 'if type == "array" then (last | .status) else .status end')
        ACO_NUMBER=$(echo $ACO_LIST | jq -r 'if type == "array" then (last | .controlOrderNumber) else .controlOrderNumber end')
        print_info "Latest order: $ACO_NUMBER, Status: $ACO_STATUS"
        
        # List all assembly control orders with workstation info
        echo "$ACO_LIST" | jq -r '.[] | "  - Order \(.controlOrderNumber): \(.status) for workstation \(.assignedWorkstationId)"'
    else
        print_result 1 "No Assembly Control orders found"
        print_info "Expected: Orders for WS-4 (Gear Assembly) and WS-5 (Motor Assembly)"
        print_info "Verify: SimAL created tasks for modules with productionWorkstationId=4,5"
    fi
fi
